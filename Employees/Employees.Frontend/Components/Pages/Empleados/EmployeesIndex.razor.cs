...
{ 
    ...
    private MudTable<Empleado> table = new();
    ...
    private async Task<TableData<Empleado>> LoadListAsync(TableState state, CancellationToken cancellationToken)
    {
        ...
        var responseHttp = await Repository.GetAsync<List<Empleado>>(url);
        if (responseHttp.Error)
        {
            var message = await responseHttp.GetErrorMessageAsync();
            Snackbar.Add(message!, Severity.Error);
            return new TableData<Empleado> { Items = [], TotalItems = 0 };
        }
        if (responseHttp.Response == null)
        {
            return new TableData<Empleado> { Items = [], TotalItems = 0 };
        }
        return new TableData<Empleado>
        {
    ...
    private async Task DeleteAsync(Empleado empleado)
    {
        var parameters = new DialogParameters
        {
            { "Message", $"Estas seguro de borrar el Empleado: {empleado.FirstName} {empleado.LastName}" }
        };
        ...
        var responseHttp = await Repository.DeleteAsync($"{baseUrl}/{empleado.Id}");
        if (responseHttp.Error)
        {
